  ...
  return (
    <div className="flex flex-col bg-white border border-gray-100 p-4 sm:p-6 md:p-8 rounded-[20px] md:rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-lg transition-all relative overflow-hidden group">
      <div className="mb-3 sm:mb-5 md:mb-6">
        <div className="h-32 sm:h-40 md:h-48 w-full bg-gray-50 rounded-lg sm:rounded-xl md:rounded-2xl mb-3 sm:mb-5 md:mb-6 flex items-center justify-center border border-gray-100 overflow-hidden relative group/slider">
          {images.length > 0 ? (
            ...
            <span className="text-zinc-400 text-[10px] sm:text-xs md:text-sm font-medium">Preview Image</span>
          )}
        </div>
        <h3 className="text-sm sm:text-xl md:text-2xl lg:text-[26px] font-bold text-black mb-1.5 sm:mb-3 line-clamp-2 leading-snug">{product.title}</h3>
        <p className="text-zinc-500 mb-2 sm:mb-4 text-[10px] sm:text-sm md:text-base line-clamp-2 sm:line-clamp-3 leading-relaxed">{product.description}</p>
      </div>
      ...
      <div className="mt-auto pt-3 sm:pt-5 md:pt-6 border-t border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0">
        <span className="text-base sm:text-2xl md:text-3xl font-bold text-black">{product.price}</span>
        <a 
...
export function Products() {
  ...
  const filteredProducts = displayProducts.filter(p => {
    ...
    if (!isNaN(numPrice) && selectedPrice !== "All Prices") {
      if (selectedPrice === "Under $50") priceMatch = numPrice < 50;
      else if (selectedPrice === "$50 - $100") priceMatch = numPrice >= 50 && numPrice <= 100;
      else if (selectedPrice === "Above $100") priceMatch = numPrice > 100;
    }
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 md:px-12 lg:px-20 py-10 md:py-16">
      <div className="text-center max-w-3xl mx-auto mb-10 md:mb-12">
        <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-zinc-900 mb-4 md:mb-6">Digital Products</h1>
        <p className="text-sm md:text-base lg:text-lg text-zinc-600 px-4 md:px-0">
          Ready-to-use, beautifully designed data solutions and templates that solve complex problems instantly.
        </p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center mb-8 gap-4 bg-gray-50 p-2 rounded-2xl md:rounded-[20px] shadow-sm border border-gray-100">
        ...
        <div className="flex w-full lg:w-auto p-1 items-center gap-2 border-t lg:border-t-0 lg:border-l border-gray-200 pt-3 lg:pt-0 lg:pl-3">
          <span className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-2 lg:ml-0">Price:</span>
          <select 
      ...
      {filteredProducts.length === 0 ? (
        <div className="text-center py-20 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
          <h3 className="text-xl font-bold text-zinc-400 mb-2">No products found</h3>
          <p className="text-zinc-500">Try adjusting your category or price filters.</p>
        </div>
      ) : (
